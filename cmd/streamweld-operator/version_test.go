...
func TestVersionNeedsNoOperatorConfiguration(t *testing.T) {
	t.Parallel()
	for _, arguments := range [][]string{
		{"--version"},
		{"-version"},
		{"--version", "--admin-token-file=/missing/version-test-token", "--admin-url=invalid", "--admin-timeout=-1s"},
	} {
		var stdout, stderr bytes.Buffer
		if code := run(arguments, &stdout, &stderr); code != 0 {
			t.Fatalf("version exit = %d, stderr = %q", code, stderr.String())
		}
		for _, field := range []string{"streamweld-operator " + version.Version, "commit " + version.Commit, "date " + version.Date} {
